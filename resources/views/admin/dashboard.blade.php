...
<div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-10">
    <div class="bg-white rounded-xl p-5 shadow-sm text-center">
        ...
        <p class="text-gray-500 text-sm mt-1">Commandes total</p>
    </div>
    <div class="bg-orange-50 rounded-xl p-5 shadow-sm text-center">
        ...
        @if($stats['pending_orders'] > 0)
            <a href="{{ route('admin.orders.index') }}"
               class="text-xs text-orange-500 hover:underline mt-1 block">
                Voir toutes →
            </a>
        @endif
    </div>
    ...
    <div class="bg-green-50 rounded-xl p-5 shadow-sm text-center">
        ...
        <p class="text-gray-500 text-sm mt-1">Clients total</p>
    </div>
    <div class="bg-purple-50 rounded-xl p-5 shadow-sm text-center">
        <p class="text-3xl font-bold text-purple-600">{{ $stats['new_users_today'] }}</p>
        <p class="text-gray-500 text-sm mt-1">🆕 Nouveaux aujourd'hui</p>
    </div>
...
<div class="grid md:grid-cols-4 gap-4 mb-8">
    ...
    <a href="{{ route('admin.orders.index') }}" class="bg-orange-500 text-white rounded-xl p-4 text-center hover:bg-orange-600 transition">
        ...
        @if($stats['pending_orders'] > 0)
            <span class="bg-white text-orange-600 text-xs font-bold px-2 py-0.5 rounded-full mt-1 inline-block">
...
<div class="bg-white rounded-xl shadow-sm overflow-hidden">
    <div class="p-5 border-b flex justify-between items-center">
        ...
        <a href="{{ route('admin.orders.index') }}" class="text-pink-600 text-sm hover:underline">Voir tout →</a>
    </div>
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y">
            ...
            <tr class="hover:bg-gray-50 {{ $order->created_at->isToday() ? 'bg-orange-50' : '' }}">
                <td class="px-5 py-3 font-mono font-bold text-pink-600">
                    {{ $order->order_number }}
                    @if($order->created_at->isToday())
                        <span class="block text-xs bg-orange-100 text-orange-600 px-1.5 py-0.5 rounded-full w-fit mt-0.5">🆕</span>
                    @endif
                </td>
